...
@login_required
def get_user_info():
    """Provides user information and usage data to the front-end after login."""
    user_data = users_collection.find_one({'_id': ObjectId(current_user.id)})
    
    # Check if usage needs to be reset (new day/month)
    today_str = datetime.utcnow().strftime('%Y-%m-%d')
    last_reset_str = user_data.get('last_usage_reset', '')
    ...
    if last_reset_str != today_str:
        users_collection.update_one(
            ...
            {'$set': {
                'usage_counts': {'messages': 0, 'webSearches': 0},
                'last_usage_reset': today_str
            }}
        )
        user_data['usage_counts'] = {'messages': 0, 'webSearches': 0}
...
def delete_account():
    ...
    try:
        # Also delete chat history associated with the user
        chat_history_collection.delete_many({'user_id': ObjectId(current_user.id)})
...
def upgrade_plan():
    ...
    try:
        ...
        print(f"PLAN_UPGRADE_ERROR: {e}")
        return jsonify({'success': False, 'error': 'Could not upgrade plan.'}), 500

@app.route('/api/live_ai', methods=['POST'])
@login_required
def live_ai():
    # Placeholder for live AI object detection logic
    # This would require a library like OpenCV and a model.
    return jsonify({'object': 'Object detection feature coming soon.'})
